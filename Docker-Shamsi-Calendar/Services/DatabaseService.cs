...

{
    public class DatabaseService
    {
        ...
        public void SaveEvent(string title, int y, int m, int d, DateTime gDate, bool isHoliday, bool isCustom = false)
        {
            using var conn = new SqliteConnection(_connectionString);
            conn.Open();
            var cmd = conn.CreateCommand();
            cmd.CommandText = @"
            INSERT INTO Events (Title, Year, Month, Day, GregorianDate, IsHoliday, IsCustom)
            VALUES ($title, $y, $m, $d, $gdate, $isHoliday, $isCustom);";
        ...
        {
            return ExecuteQuery("SELECT Id, Title, Year, Month, Day, GregorianDate, IsHoliday, IsCustom FROM Events WHERE Year = $y AND Month = $m;",
        ...
        public List<CalendarEvent> GetEventsForIcs()
        {
            ...
            return ExecuteQuery("SELECT Id, Title, Year, Month, Day, GregorianDate, IsHoliday, IsCustom FROM Events WHERE Year >= $min AND Year <= $max;",
        ...
        private List<CalendarEvent> ExecuteQuery(string query, Dictionary<string, object> parameters)
        {
            ...
            while (reader.Read())
            {
                list.Add(new CalendarEvent(reader.GetInt32(0), reader.GetString(1), reader.GetInt32(2), reader.GetInt32(3), reader.GetInt32(4), DateTime.Parse(reader.GetString(5)), reader.GetInt32(6) == 1, reader.GetInt32(7) == 1));
            }
            return list;
        }
    }
